function [y]=delete_unstable_status_cal(x,n,pos_array,len_array,threshold_a_unstable,threshold_a_stable,threshold_b_unstable,threshold_b_stable)
%该函数删除信号中不稳定的状态
...
for i=2:n-1
    if ((len_array(i) < threshold_a_unstable) &&  (len_array(i-1) >= threshold_a_stable) && (y(pos_array(i)) ~= 2)) || ((len_array(i) < threshold_b_unstable) &&  (len_array(i-1) >= threshold_b_stable) &&  (len_array(i+1) >= threshold_b_stable) && (y(pos_array(i)) ~= 2))
