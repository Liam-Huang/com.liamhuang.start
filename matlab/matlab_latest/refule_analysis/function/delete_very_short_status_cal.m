function [y]=delete_very_short_status_cal(x,n,pos_array,len_array,threshold_a_unstable,threshold_a_stable,threshold_b_unstable,threshold_b_stable, status_refule, status_steal)
%该函数删除信号中不稳定的状态
...
for i=2:n-1
    if ((len_array(i) < threshold_b_unstable) &&  (len_array(i-1) >= threshold_b_stable) && (y(pos_array(i)) ~= status_refule) && (y(pos_array(i)) ~= status_steal)) || ((len_array(i) < threshold_a_unstable) &&  (len_array(i-1) >= threshold_a_stable) &&  (len_array(i+1) >= threshold_a_stable) && (y(pos_array(i)) ~= status_refule) && (y(pos_array(i)) ~= status_steal))
