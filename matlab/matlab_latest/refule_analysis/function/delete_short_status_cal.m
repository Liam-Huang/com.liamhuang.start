function [y]=delete_short_status_cal(x,n,pos_array,len_array,threshold_unstable,threshold_stable,status_refule,status_steal)
%该函数删除信号中不稳定的状态
...
for i=2:n-1
    if ( (y(pos_array(i)) ~= status_refule) && (y(pos_array(i)) ~= status_steal) && (len_array(i) < threshold_unstable) &&  (len_array(i-1) >= threshold_stable) && (len_array(i+1) >= threshold_stable) )
